...
static Header basewf[NRQUICKLISTS]; /* empty list to get started */
...
void init_freelists()
{
	...
	{
		freelists[i] = &basewf[i];
		freelists[i]->s.next = NULL;
		freelists[i]->s.size = 0;
	}

	freelists[NRQUICKLISTS-1]->s.next = &basewf[NRQUICKLISTS-1];
}
...
static Header* allocate_blocks(Header * list, unsigned int units, unsigned int blocks)
{
	...
	for(i = 0; i < blocks; i++)
	{
		...
		qffree((void*)(new_slot+1));
	}
	return list;
}

/* malloc: general-purpose storage allocator */
void *qfmalloc(size_t nbytes)
{
...
void *qfrealloc(void * oldpointer, size_t new_size)
{
	/* If pointer is NULL, then the call is equivalent to malloc */
	if (oldpointer == NULL)
	{
		return qfmalloc(new_size);
	}
	...
	void * newpointer = qfmalloc(new_size);
