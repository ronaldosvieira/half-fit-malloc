...
/* morecorewf: ask system for more memory */
static Header *morecorewf(unsigned int number_of_units)
{
    ...
    wffree((void *)(new_slot+1));
    return freelist;
}

/* malloc: general-purpose storage allocator */
void *wfmalloc(size_t nbytes)
{
            ...
            current_pointer  = current_pointer->s.next)
    {
        ...
        { /* back to the beginning */
            ...
            {
                if((current_pointer = morecorewf(nunits)) == NULL)
                { /* no memory left*/
                    return NULL;
                }
            }else{
...
void *wfrealloc(void * oldpointer, size_t new_size)
{
    /* If pointer is NULL, then the call is equivalent to malloc */
    if (oldpointer == NULL)
    {
        return wfmalloc(new_size);
    }
    ...
    void * newpointer = wfmalloc(new_size);
