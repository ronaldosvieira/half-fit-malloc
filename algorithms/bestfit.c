...
static Header *freelistbf = NULL;


/* morecorebff: ask system for more memory */
static Header *morecorebff(unsigned int number_of_units)
{
    ...
    bffree((void *)(new_slot+1));
    return freelistbf;
}

/* malloc: general-purpose storage allocator */
void *bfmalloc(size_t nbytes)
{
    ...
    if (freelistbf== NULL) 
    {
        base.s.next = freelistbf = &base;
        base.s.size = 0;
    }

    previous_pointer = freelistbf;
           ...
           current_pointer  = current_pointer->s.next)
    {
        ...
        if (current_pointer == freelistbf) 
        { /* back to the beginning */
            if(smallest == NULL || smallest->s.size < nunits)
            {
                if(( current_pointer = morecorebff(nunits)) == NULL)
                { /* no memory left*/
                    return NULL;
                }
            }else{
    ...
    {/* it fits perfectly in the block*/
        ...
        if(smallest == freelistbf)
        {
            freelistbf = smallest->s.next;
        }
    }
    else
    {
...
void *bfrealloc(void * oldpointer, size_t new_size)
{
    /* If pointer is NULL, then the call is equivalent to malloc */
    if (oldpointer == NULL)
    {
        return bfmalloc(new_size);
    }
    ...
    void * newpointer = bfmalloc(new_size);
